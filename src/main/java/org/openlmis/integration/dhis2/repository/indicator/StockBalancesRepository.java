...
public class StockBalancesRepository {
  ...
  /**
   * Retrieves current stock on hand from stockmanagement.
   */
  public Long findStockOnHand(@Param(CURRENT_DATE) ZonedDateTime currentDate,
                              @Param(ORDERABLE) String orderable,
                              @Param(FACILITY) String facility) {
    Query query = entityManager.createNativeQuery(
        "(SELECT cal.stockonhand "
